...
1. 在 Application Insights 资源中，单击应用程序仪表板，以查看提供的默认仪表板的性能数据。
   ...
   > **注意：** 仪表板可能需要一些时间以完全加载和显示所有性能数据。 等待仪表板完全呈现后再继续，以确保后续练习中的最佳体验。
...
1. 添加其他指标请重复此步骤。
   - CPU 时间（计数）
   - 请求（Average）
1. 在“指标”图表区域中，单击图表右上角的“固定到仪表板”按钮（固定图标）****。
1. 在显示的“固定到仪表板”对话框中：****
   - 选择“新建”以创建新的仪表板****
   - 输入仪表板名称：`MonitoringLab Dashboard`****
   - 选择适当的订阅
   - 选择仪表板类型：
     - 公共****：仪表板对组织中的所有用户可见。 这适用于需要共享可见性的场景。
     - 专用****：仪表板仅对你可见。 建议将此项用于实验室练习或数据可见性应受到限制的场景。
   - 单击“创建并固定”****
1. 创建仪表板并固定指标后，单击顶部菜单中的“保存”以保存仪表板****。
1. 通过单击 Azure 门户左侧面板中的“仪表板”图标，导航到仪表板，或在顶部搜索栏中搜索“仪表板”****。
1. 从仪表板列表中选择新创建的 MonitoringLab 仪表板****。
